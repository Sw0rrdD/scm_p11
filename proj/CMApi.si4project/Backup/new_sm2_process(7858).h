...
#ifndef _SM2_PROCESS_H
#define _SM2_PROCESS_H
...
修改历史:
    1. 2017年3月30日	陈位仅		函数改造
*******************************************************************************/
int SM2_Encrypt_Smvc(sc_session_t *session, struct sc_pkcs15_card *p15_card, CK_ULONG key_obj_mem_addr, BYTE *pbPlainText,
...
修改历史:
    1. 2017年3月30日	陈位仅		函数改造
*******************************************************************************/
int SM2_Decrypt_Smvc(sc_session_t *session, struct sc_pkcs15_card *p15_card, CK_ULONG key_obj_mem_addr, BYTE *pbCipherText,
 ...
 */
int SM2_Sign_Smvc(sc_session_t *session, struct sc_pkcs15_card *p15_card, CK_ULONG key_obj_mem_addr, u8 *inData, unsigned long inDataLength,
 ...
 * FIXME:该接口使用的是sm2提供的hash算法，这个接口，暂时保留，以便以后扩展。
 */
int SM2_Verify_Smvc(sc_session_t *session, struct sc_pkcs15_card *p15_card, CK_ULONG key_obj_mem_addr, u8 *inData, unsigned long inDataLength,
		u8 *inOrOutData, unsigned long *inOrOutDataLength);

/*
 * 调用sm2算法进行签名，输入数据为数据值，该函数内部会调用sm3进行hash，对hash值进行签名
 */
int SM2_Sign_Direct(sc_session_t *session, struct sc_pkcs15_card *p15_card, CK_ULONG key_obj_mem_addr, u8 *inData, unsigned long inDataLength,
		u8 *inOrOutData, unsigned long *inOrOutDataLength);
