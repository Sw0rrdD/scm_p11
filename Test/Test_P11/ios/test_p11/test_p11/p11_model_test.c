...
CK_RV p11_mode_crypt_sm2(CK_SESSION_HANDLE hSession)
{
...

	{
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Generate Key Pair Failed:%08x\n", rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Sign Initialize Key Failed:%08x\n", rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Verify Initialize Key Failed:%08x\n", rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Verify Failed:%08x\n", rv);
			return rv;
		}
	}

	{
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Encrypt Initialize Key Failed:%08x\n", rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Encrypt Failed:%08x\n", rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Decrypt Initialize Key Failed:%08x\n", rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Decrypt Failed:%08x\n", rv);
			return rv;
		}

		if (0 != memcmp(srandom, signbuffer, sizeof(srandom)))
		{
			printf("SM2 Crypto Failed\n");
		}
	}

	{
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Generate Key Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Wrap Key Failed:%08x\n", rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Unwrap Key Failed:%08x\n", rv);
			return rv;
		}
	}
...
CK_RV p11_mode_crypt_sm3(CK_SESSION_HANDLE hSession)
{
	...
	if (rv != CKR_OK)
	{
		printf("PKCS11 Generate SM4 Secret Key Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}

	{
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Hash Init Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Hash Update Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Hash Final Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
...

	{
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Hmac Init Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Hmac Update Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Hmac Final Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
...

	{
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Cmac Init Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
...
CK_RV p11_mode_crypt_sm4(CK_MECHANISM_TYPE mode, CK_SESSION_HANDLE hSession)
{
	...
	
	{
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Encrypt Init Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Encrypt Update Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Encrypt Final Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Decrypt Init Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Decrypt Update Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if (rv != CKR_OK)
		{
			printf("PKCS11 Decrypt Final Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
...
int p11_model_test()
{
	...
	{
		printf("PKCS11 Get Function List Failed %08x\n", (CK_UINT)rv);
		return rv;
	}
	...
	if (rv != CKR_OK)
	{
		printf("PKCS11 Initialize Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	...
	if(rv != CKR_OK)
	{
		printf("PKCS11 Get Info Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	else
	{
		printf("CryptokiVersion Major: %02x\n", info.cryptokiVersion.major);
		printf("CryptokiVersion Minor: %02x\n", info.cryptokiVersion.minor);
		printf("LibraryVersion Major: %02x\n", info.libraryVersion.major);
		printf("LibraryVersion Minor: %02x\n", info.libraryVersion.minor);
	}

	rv = C_GetSlotList(CK_FALSE, pSlotList, &ulSlotCount);
	if(rv != CKR_OK)
	{
		printf("PKCS11 Get Slot List Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	else
	{
	...
	{
		printf("PKCS11 Get Token Info Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	...
	if (rv != CKR_OK)
	{
		printf("PKCS11 Get Mechanism List Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	...
	for (mech_idx = 0; mech_idx < mech_count; mech_idx++)
	{
		...
		if (rv == CKR_OK)
		{
			if (mech_info.ulMinKeySize || mech_info.ulMaxKeySize)
			{
				...
				{
					printf("%li", mech_info.ulMinKeySize);
				}
				...
				{
					printf("%li", mech_info.ulMaxKeySize);
				}
			...
			if (mech_info.flags)
			{
				printf(", other flags=0x%x", (unsigned int)mech_info.flags);
			}
		}
		else
		{
			printf("PKCS11 Get mechianism Info Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		printf("\n");
	}
	...
	if(rv != CKR_OK)
	{
		printf("PKCS11 Open Session Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	...
	if(rv != CKR_OK)
	{
		printf("PKCS11 Login Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	...
	if(rv != CKR_OK)
	{
		printf("PKCS11 Set Pin Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	...
	if (rv != CKR_OK)
	{
		printf("PKCS11 Get Session Info Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	else
	{
		printf("sessioninfo->slotID %d\n", (int)session_info.slotID);
		printf("sessioninfo->state %d\n", (int)session_info.state);
		printf("sessioninfo->flag %d\n", session_info.flags);
	}

	{
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Create Public Key Object Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Copy Public Key Object Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Destroy Public Key Object Failed:%08x\n", (CK_UINT)rv);
			//return rv;
		}
	}

	{
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Create Private Key Object Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Destroy Private Key Object Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
	}

	{
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Create Secret Key Object Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Copy Secret Key Object Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Destroy Secret Key Object Failed:%08x\n", (CK_UINT)rv);
			//return rv;
		}
	}

	{
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Create Certificate Object Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Copy Certificate Key Object Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Destroy Certificate Object Failed:%08x\n", (CK_UINT)rv);
			//return rv;
		}
	}

	{
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Create Data Object Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Copy Data Key Object Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Destroy Data Object Failed:%08x\n", (CK_UINT)rv);
			//return rv;
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Get Object Size Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Get Object Attribute Value Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		else
		{
			...
			{
				printf("PKCS11 Get Object Attribute Value Is Invald!\n");
			}
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Set Object Attribute Value Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Find Objects Init Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Find Objects Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
		...
		if(rv != CKR_OK)
		{
			printf("PKCS11 Find Objects Final Failed:%08x\n", (CK_UINT)rv);
			return rv;
		}
	}
	...
	if (rv != CKR_OK)
	{
		printf("PKCS11 SM4 ECB Crypto Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	...
	if (rv != CKR_OK)
	{
		printf("PKCS11 SM4 CBC Crypto Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	...
	if (rv != CKR_OK)
	{
		printf("PKCS11 SM4 OFB Crypto Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	...
	if (rv != CKR_OK)
	{
		printf("PKCS11 Digest Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	...
	if (rv != CKR_OK)
	{
		printf("PKCS11 SM2 Test Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	...
	if (rv != CKR_OK)
	{
		printf("PKCS11 User Logout Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	...
	if (rv != CKR_OK)
	{
		printf("PKCS11 Close All Sessions Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
	...
	if (rv != CKR_OK)
	{
		printf("PKCS11 Finalize Failed:%08x\n", (CK_UINT)rv);
		return rv;
	}
    return rv;
}
